import {getMyProfile, login, registration} from "../../http/userAPI";
    ...
    return async (dispatch) => {
        try {
            ...
            if (e.response.status === 400) {
                dispatch(setError('Заполните пустые поля'))
            } else {
                dispatch(setError('Логин или пароль неправильный'))
            }
            dispatch(toggleIsFetching(false));
        }
    }
...
export  const checkToken = (username) =>{
    return async () =>{
        try {
            await getMyProfile(username)
        }catch (e) {
